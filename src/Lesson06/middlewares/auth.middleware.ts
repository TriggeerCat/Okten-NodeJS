...
class AuthMiddleware {
    public async checkAccessToken(req: Request, res: Response, next: NextFunction) {
        ...
        if (!authorizationHeader) {
            next(new ApiError("No auth header provided", StatusCodeEnum.UNAUTHORIZED));
            return;
        }
        const accessToken = authorizationHeader?.split(" ")[1];
        if (!accessToken) {
            next(new ApiError("No token provided", StatusCodeEnum.UNAUTHORIZED));
            return;
        }
        const tokenPayload = tokenService.verifyToken(accessToken ?? " ", "access");
        if (!(await tokenService.doesTokenExist(accessToken, "access"))) {
            next(new ApiError("Invalid token provided", StatusCodeEnum.UNAUTHORIZED));
            return;
        }
    ...
    public async checkRefreshToken(req: Request, res: Response, next: NextFunction) {
        ...
        if (!refreshToken) {
            next(new ApiError("No token provided", StatusCodeEnum.FORBIDDEN));
            return;
        }
        ...
        if (!(await tokenService.doesTokenExist(refreshToken, "refresh"))) {
            next(new ApiError("Invalid token provided", StatusCodeEnum.FORBIDDEN));
            return;
        }
        if (req.res?.locals) req.res.locals.tokenPayload = tokenPayload;
        next();
